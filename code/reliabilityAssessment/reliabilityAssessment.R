### This script will be used to assess the reliability of the EEG IdEmo behavioral data
...
# ---- prepare-reliability-analyses ---------------------
## I am going to look at number correct across the four administrations
reli.data <- as.data.frame(for.irt3)
reli.data$ID <- as.character(for.irt[,1])
names(reli.data)[1:96] <- strSplitMatrixReturn(colnames(for.irt)[2:97], "_")[,2]
## Now obtain the correct responses across all 96 
reli.data$totalCorrectAll <- rowSums(reli.data[,1:96], na.rm = T)
reli.data$totalCorrectCry <- rowSums(reli.data[,grep(pattern = "^C", names(reli.data))], na.rm = T)
reli.data$totalCorrectHap <- rowSums(reli.data[,grep(pattern = "^H", names(reli.data))], na.rm = T)
reli.data$totalCorrectNeu <- rowSums(reli.data[,grep(pattern = "^N", names(reli.data))], na.rm = T)
reli.data$totalCorrectUnh <- rowSums(reli.data[,grep(pattern = "^U", names(reli.data))], na.rm = T)

## Now add an observation count index
reli.data$adminCount <- NA
row.id <- unique(reli.data$ID)
for(i in row.id){
  reli.data[which(reli.data$ID==i),"adminCount"] <- 1:4
}

## Now spread the data so each row has 4 correct observations
all.cor <- reli.data %>% tidyr::pivot_wider(., id_cols = ID, values_from=c(totalCorrectAll, totalCorrectCry, totalCorrectHap, totalCorrectNeu, totalCorrectUnh), names_from=adminCount) %>% 
  mutate(., allMean=rowMeans(select(., contains("All")))) %>% 
  mutate(., cryMean=rowMeans(select(., contains("Cry")))) %>% 
  mutate(., hapMean=rowMeans(select(., contains("Hap")))) %>% 
  mutate(., unhMean=rowMeans(select(., contains("Neu")))) %>% 
  mutate(., neuMean=rowMeans(select(., contains("Unh")))) 

## Now estimate the correlation matrix across the correct and incorrect responses
...
# Now plot these
library(GGally)
all.plot <- all.cor %>% select(., contains("All")) %>% ggpairs(., upper = list(continuous = wrap("cor", family="sans")))
cry.plot <- all.cor %>% select(., contains("Cry")) %>% ggpairs(., upper = list(continuous = wrap("cor", family="sans")))
hap.plot <- all.cor %>% select(., contains("Hap")) %>% ggpairs(., upper = list(continuous = wrap("cor", family="sans")))
unh.plot <- all.cor %>% select(., contains("Unh")) %>% ggpairs(., upper = list(continuous = wrap("cor", family="sans")))
neu.plot <- all.cor %>% select(., contains("Neu")) %>% ggpairs(., upper = list(continuous = wrap("cor", family="sans")))
all.vals <- all.cor %>% select(., contains("Correct")) %>% ggpairs(., upper = list(continuous = wrap("cor", family="sans")))
