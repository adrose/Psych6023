...
interact2 | f xwith P2amp ;
interact3 | f xwith P2lat ;
interact4 | f xwith P2int ;
...
interact2 (gammaalpha);",
    paste0("\nV",item_num," on P2lat"),
    "\ninteract3 (deltaalpha);",
    paste0("\nV",item_num," on P2int"),
    "\ninteract4 (lambdaalpha);
...
# ---- tweak-data --------------------------------------------------------------
#as.numeric(scale(irt_dat$P2crymaxCareLatpost))
cat_cut <- 0.5
mp_irt_dat <- irt_dat %>%
  mutate(
...
nures_nonuniform <- nures

## Save these models for export
save(nures_nonuniform, file="./data/nunimModels.RData")
